...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "${var.namespace}-public-${count.index}" }
}
...
resource "aws_subnet" "private" {
  ...
  tags = { Name = "${var.namespace}-private-${count.index}" }
}
...
resource "aws_eip" "nat" {
  domain = "vpc"
  tags   = { Name = "${var.namespace}-nat-eip" }
}

resource "aws_nat_gateway" "main" {
  allocation_id = aws_eip.nat.id
  subnet_id     = aws_subnet.public[0].id
  tags          = { Name = "${var.namespace}-nat" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "${var.namespace}-private-rt" }
}
...
resource "aws_security_group" "ecs_tasks" {
  ...
  tags = { Name = "${var.namespace}-ecs-tasks" }
}
