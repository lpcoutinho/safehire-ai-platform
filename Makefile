...
up: ## Inicia todos os serviços
	...
	@echo "SQS Management: http://localhost:9327"
...
db-shell: ## Abre psql no RDS (Floci)
	docker compose exec floci psql -h localhost -p 5432 -U safehire -d safehire || docker compose exec floci psql -h localhost -p 5433 -U safehire -d safehire

redis-shell: ## Abre redis-cli no ElastiCache (Floci)
	docker compose exec floci redis-cli -h localhost -p 6379 || docker compose exec floci redis-cli -h localhost -p 6380
